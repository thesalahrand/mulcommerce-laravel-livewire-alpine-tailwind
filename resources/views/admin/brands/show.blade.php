<x-admin-app-layout>
  @php
    $breadcrumbItems = [['text' => __('Dashboard'), 'link' => route('admin.dashboard')], ['text' => __('Brands'), 'link' => url()->previous() != url()->current() && strpos(url()->previous(), route('admin.brands.index')) !== false ? url()->previous() : route('admin.brands.index')], ['text' => __('Show'), 'link' => route('admin.brands.show', $brand->id)]];
  @endphp
