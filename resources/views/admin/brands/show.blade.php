<x-admin-app-layout>
  @php
    $breadcrumbItems = [['text' => __('Dashboard'), 'link' => route('admin.dashboard')], ['text' => __('Brands'), 'link' => strpos(url()->previous(), route('admin.brands.index') . '?') !== false ? url()->previous() : route('admin.brands.index')], ['text' => $brand->name, 'link' => route('admin.brands.show', $brand->id)]];
  @endphp
