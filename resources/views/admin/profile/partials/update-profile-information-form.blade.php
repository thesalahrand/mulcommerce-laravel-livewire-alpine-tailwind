<section class="w-full max-w-3xl">
  ...
  <form method="post" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data">
    ...
    {{-- Photo, Address --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
      <div x-data="imageViewer('{{ $user->photo }}')">
        <x-input-label for="photo" :value="__('Photo')" />
        <x-file-input id="photo" name="photo" type="file" accept=".jpg, .jpeg" @change="fileChosen" />
        <div class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="user_avatar_help">Your photo should be in JPG
          format (max
          1 MB)</div>
        <x-input-error :messages="$errors->get('photo')" />
        <img :src="imageUrl" class="rounded mt-2 w-32 h-32 object-fit" alt="user-photo">
      </div>
      <div>
