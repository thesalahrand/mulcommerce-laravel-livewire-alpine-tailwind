<x-admin-app-layout>
  @php
    $breadcrumbItems = [['text' => __('Dashboard'), 'link' => route('admin.dashboard')], ['text' => __('Subcategories'), 'link' => str()->contains(url()->previous(), route('admin.subcategories.index') . '?') ? url()->previous() : route('admin.subcategories.index')], ['text' => $subcategory->name, 'link' => route('admin.subcategories.show', $subcategory->id)]];
  @endphp
