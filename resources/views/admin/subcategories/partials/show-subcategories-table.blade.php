        ...
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
          ...
          <td class="px-6 py-4">
            <div class="flex items-center space-x-1">
              <x-td-action-button href="{{ route('admin.subcategories.show', $subcategory->id) }}"><x-icons.eye
                  class="w-5 h-5" /></x-td-action-button>
              <x-td-action-button href="{{ route('admin.subcategories.edit', $subcategory->id) }}"><x-icons.pencil
                  class="w-5 h-5" /></x-td-action-button>
              <x-td-action-button class="cursor-pointer"
                @click="show, idToDelete = {{ $subcategory->id }}"><x-icons.trash
                  class="w-5 h-5" /></x-td-action-button>
            </div>
          </td>
        </tr>
