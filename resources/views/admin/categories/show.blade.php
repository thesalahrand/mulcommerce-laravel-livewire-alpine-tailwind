<x-admin-app-layout>
  @php
    $breadcrumbItems = [['text' => __('Dashboard'), 'link' => route('admin.dashboard')], ['text' => __('Categories'), 'link' => strpos(url()->previous(), route('admin.categories.index') . '?') !== false ? url()->previous() : route('admin.categories.index')], ['text' => $category->name, 'link' => route('admin.categories.show', $category->id)]];
  @endphp
      ...
      <div
        ...
        <div class="flex justify-end items-center space-x-1 mb-6">
          <a class="text-gray-500 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
            href="{{ route('admin.categories.edit', $category->id) }}"><x-icons.pencil class="w-5 h-5" /></a>
        ...
        @include('admin.categories.partials.show-category-details-card')
      </div>
    </div>
  </div>
</x-admin-app-layout>
