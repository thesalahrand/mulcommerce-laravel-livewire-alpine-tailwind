<x-admin-app-layout>
  @php
    $breadcrumbItems = [['text' => __('Dashboard'), 'link' => route('admin.dashboard')], ['text' => __('Categories'), 'link' => route('admin.categories.index')]];
  @endphp
  ...
  <div x-data="{ idToDelete: '' }">
    <div x-data="customModalHandler('#confirm-category-deletion-modal', false)">
      ...
      <div class="mb-4 flex flex-1 flex-col sm:flex-row space-y-2 sm:space-y-0 justify-between sm:items-center">
        <form method="GET" class="flex items-center w-full sm:w-1/2">
          ...
          <x-text-input id="search" type="search" name="s" :value="old('search', request('s'))" autofocus
            placeholder="{{ __('Search by category ID, name, and slug') }}" autocomplete="search" />
      ...
      @include('admin.categories.partials.show-categories-table')
    </div>
  </div>
</x-admin-app-layout>
